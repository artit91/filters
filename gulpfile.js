...
(function () {
    ...
    gulp.task('sass:chrome', function () {
        return gulp.src('.tmp/chrome.scss')
            .pipe(gulp.sass({includePaths: ['vendor/bootstrap-sass/'], file: 'chrome.css'}).on('error', gulp.sass.logError))
            .pipe(gulp.dest('.tmp/'));
    });
    ...
    gulp.task('build:chrome', ['clean:chrome', 'common'], function (done) {
        return gulp.runSequence('jslint:chrome', 'concat:chrome_sass', 'sass:chrome', 'copy:chrome', 'concat:background', done);
    });
